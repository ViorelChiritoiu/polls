...
public class PollService {
    ...
    public PagedResponse<PollResponse> getPollsVotedBy(String username, UserPrincipal currentUser, int page, int size) {
        ...
        Pageable pageable = PageRequest.of(page, size, Sort.Direction.DESC, "createdAt");
        Page<Long> userVotePollIds = voteRepository.findVotedPollIdsByUserId(user.getId(), pageable);

        if(userVotePollIds.getNumberOfElements() == 0) {
            return new PagedResponse<>(Collections.emptyList(), userVotePollIds.getNumber(), userVotePollIds.getSize(),
                    userVotePollIds.getTotalElements(), userVotePollIds.getTotalPages(), userVotePollIds.isLast());
        }

        // Retrieve all poll details from the voted pollIds.
        List<Long> pollIds = userVotePollIds.getContent();
        List<Poll> polls = pollRepository.findByIdIn(pollIds, Sort.by(Sort.Direction.DESC, "createdAt"));
        ...
        return new PagedResponse<>(pollResponses, userVotePollIds.getNumber(), userVotePollIds.getSize(),
                userVotePollIds.getTotalElements(), userVotePollIds.getTotalPages(), userVotePollIds.isLast());
    }
